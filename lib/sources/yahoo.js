...
function toIsoTimestampFromUnixSeconds(seconds) {
...
export function buildYahooChartUrl(yahooTicker, request = { range: '400d' }) {
  ...
  params.set('interval', request.interval || '1d');
...
export class YahooRateLimitError extends Error {
  constructor(yahooTicker, retryAfter = null) {
    super(`Yahoo fetch failed for ${yahooTicker}: 429 Too Many Requests`);
    this.name = 'YahooRateLimitError';
    this.code = 'YAHOO_RATE_LIMIT';
    this.status = 429;
    this.ticker = yahooTicker;
    this.retryAfter = retryAfter;
  }
}

export function parseYahooDailyChartPayload(yahooTicker, payload) {
...
export function parseYahooIntradayChartPayload(yahooTicker, payload) {
  const result = payload?.chart?.result?.[0];
  const error = payload?.chart?.error;
  ...
  return result.timestamp.map((timestamp, index) => ({
    candle_at: toIsoTimestampFromUnixSeconds(timestamp),
    session_date: toIsoDateFromUnixSeconds(timestamp),
    open: quote.open?.[index] ?? null,
    high: quote.high?.[index] ?? null,
    low: quote.low?.[index] ?? null,
    close: quote.close?.[index] ?? null,
...
export async function fetchYahooDailyCandles(yahooTicker, request = { range: '400d' }) {
  ...
  if (!response.ok) {
    if (response.status === 429) {
      throw new YahooRateLimitError(yahooTicker, response.headers.get('retry-after'));
    }
    throw new Error(`Yahoo fetch failed for ${yahooTicker}: ${response.status} ${response.statusText}`);
  }

  const payload = await response.json();
  return parseYahooDailyChartPayload(yahooTicker, payload);
}

export async function fetchYahooIntradayCandles(
  yahooTicker,
  request = { range: '2mo', interval: '60m' }
) {
  const url = buildYahooChartUrl(yahooTicker, {
    ...request,
    interval: request.interval || '60m',
  });
  ...
  if (!response.ok) {
    throw new Error(`Yahoo intraday fetch failed for ${yahooTicker}: ${response.status} ${response.statusText}`);
  }

  const payload = await response.json();
  return parseYahooIntradayChartPayload(yahooTicker, payload);
}
