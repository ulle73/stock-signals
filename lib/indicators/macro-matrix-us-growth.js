import { colorBucketFromHeatmapScore, getMacroMatrixColorMetrics, smoothMacroMatrixColorCells } from '../utils/macro-matrix-colors.js';
...
export const MACRO_MATRIX_US_GROWTH_DEFINITIONS = [
  ...
  { key: 'oecd_leading_indicator_us_yoy_proxy', label: 'OECD Leading Indicator Y/Y %', category: 'leading_indicators', seriesId: 'USALOLITOAASTSAM', frequency: 'monthly', transform: 'yoy', direction: 'higher_is_better' },
...
function computeTransformedValue(periodDate, rawValue, rawValueByPeriod, transform) {
  ...
  if (transform === 'raw') return rawValue;
...
function getDefinitionColorConfig(definition) {
  if (definition.direction === 'pmi') {
    return { type: 'pmi', direction: 'pmi', neutralLevel: 50, normalMonthlyMove: 1, pmiLevelScale: 5, pmiChangeWeight: 0.25 };
  }
  ...
  if (definition.direction === 'inflation') {
    return { type: 'level_momentum', direction: 'lower_is_better', neutralLevel: 2.5, levelScale: 1.25, normalMonthlyMove: 0.5, changeWeight: 0.35 };
  }
  ...
  return { type: 'level_momentum', direction: 'higher_is_better', neutralLevel: 0, levelScale: 12, normalMonthlyMove: 2 };
}

function computeColorMetrics(definition, transformedValue, previousTransformedValue) {
  if (definition.direction === 'inventory') {
    return getMacroMatrixColorMetrics({
  ...
  return getMacroMatrixColorMetrics({
    value: transformedValue,
    previousValue: previousTransformedValue,
    ...getDefinitionColorConfig(definition),
  });
}
...
export function transformMacroSeriesToMonthlyObservations(rows, definition) {
  ...
  return monthlyRows.map((row) => {
    ...
    const metrics = computeColorMetrics(definition, transformedValue, previousTransformedValue);
...
function applyInventoryOverride(observation, retailObservation) {
  if (!observation || observation.transformedValue === null) return observation;
  ...
  const heatmapScore = directionScore;
  return {
    ...observation,
    directionScore,
    heatmapScore,
...
export function classifyMacroGrowthRegime({ macroGrowthScore, percentPositive, percentNegative }) {
  ...
  if (macroGrowthScore <= -0.5 && percentNegative >= 70) {
    return { macroGrowthRegime: 'macro_stress', macroGrowthRiskAction: 'GO_TO_CASH' };
  }

  if (macroGrowthScore >= 0.35 && percentPositive >= 60) {
    return { macroGrowthRegime: 'expansion_improving', macroGrowthRiskAction: 'RISK_ON' };
  }

  if (macroGrowthScore >= 0.1 && percentPositive >= 50) {
    return { macroGrowthRegime: 'growth_stable_positive', macroGrowthRiskAction: 'NEUTRAL_TO_RISK_ON' };
  }

  if (macroGrowthScore > -0.1) {
    return { macroGrowthRegime: 'mixed_neutral', macroGrowthRiskAction: 'NEUTRAL' };
  }

  if (macroGrowthScore <= -0.35 || percentNegative >= 60) {
    return { macroGrowthRegime: 'broad_macro_contraction', macroGrowthRiskAction: 'REDUCE_RISK' };
  }

  return { macroGrowthRegime: 'growth_deteriorating', macroGrowthRiskAction: 'NO_NEW_BUYS' };
}

export function buildMacroMatrixUsGrowth(observationsByKey, { monthCount = DEFAULT_MONTH_COUNT, quarterCount = DEFAULT_QUARTER_COUNT } = {}) {
  ...
  const summaryByMonth = months.map((periodDate, index) => {
    ...
    const macroGrowthScore = validRowCount ? normalizeNumber(validCells.reduce((sum, cell) => sum + cell.directionScore, 0) / validRowCount) : null;
