...
const RYD_OBV_KEYS = Object.freeze([
  'ryd_obv',
  'ryd_obv_zscore_80',
  'ryd_obv_buy_signal',
  'ryd_obv_sell_signal',
...
function normalizedRydSignal(value) {
  const signal = String(value ?? 'none').trim().toLowerCase();
  return RYD_OBV_SIGNAL_VALUES.has(signal) ? signal : 'none';
}
...
function normalizedBar(row) {
  ...
  const hasRydObvFields = RYD_OBV_KEYS.some((key) => Object.hasOwn(row, key));
  if (hasRydObvFields) {
    ...
    bar.ryd_obv_signal = normalizedRydSignal(row.ryd_obv_signal);
  }
