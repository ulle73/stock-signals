...
export function runStatePeriodStudy(config, { bars, registry }) {
  ...
  for (let index = 0; index < bars.length; index += 1) {
    ...
    const end = resolveStatePeriodEnd(index, bars, config);
    if (!end) {
      continue;
    }

    const exitIndex = end.endSignalIndex + exitDelayBars;
    if (!bars[exitIndex] || exitIndex <= entryIndex) {
      continue;
    }
    ...
    periods.push({
      ...
      end_signal_date: bars[end.endSignalIndex].date,
