...
const DEFAULT_CHANGE_WEIGHT = 0.2;
const DEFAULT_PMI_NEUTRAL_LEVEL = 50;
const DEFAULT_PMI_LEVEL_SCALE = 5;
const DEFAULT_PMI_CHANGE_WEIGHT = 0.25;
const DEFAULT_MAX_COLOR_STEP_CHANGE = 1;

const BUCKET_TO_STEP = new Map([
  ['red-5', -5],
  ['red-4', -4],
  ['red-3', -3],
  ['red-2', -2],
...
const STEP_TO_BUCKET = new Map([...BUCKET_TO_STEP.entries()].map(([bucket, step]) => [step, bucket]));
...
export function colorStepFromBucket(bucket) {
  if (bucket === 'missing') return null;
  return BUCKET_TO_STEP.get(bucket) ?? 0;
}

export function colorBucketFromStep(step) {
  if (step === null || step === undefined || !Number.isFinite(Number(step))) return 'neutral';
  return STEP_TO_BUCKET.get(clamp(Math.round(Number(step)), -5, 5)) ?? 'neutral';
}

export function smoothMacroMatrixColorCells(cells, { maxStepChange = DEFAULT_MAX_COLOR_STEP_CHANGE } = {}) {
  let previousStep = null;
  const resolvedMaxStepChange = Math.max(1, Math.abs(Number(maxStepChange)) || DEFAULT_MAX_COLOR_STEP_CHANGE);

  return cells.map((cell) => {
    const targetStep = colorStepFromBucket(cell.colorBucket);

    if (targetStep === null || cell.transformedValue === null || cell.transformedValue === undefined) {
      return cell;
    }
    ...
    if (previousStep === null) {
      ...
      return {
        ...cell,
        colorStep: targetStep,
        rawColorBucket: cell.colorBucket,
      };
    }

    const smoothedStep = clamp(
      targetStep,
      previousStep - resolvedMaxStepChange,
      previousStep + resolvedMaxStepChange
    );
    previousStep = smoothedStep;

    return {
      ...cell,
      colorStep: smoothedStep,
      rawColorBucket: cell.colorBucket,
      colorBucket: colorBucketFromStep(smoothedStep),
    };
  });
}
