...
function isNewTradingWeek(previousDate, currentDate) {
...
function shouldRebalance(strategy, previousExecutionDate, executionDate) {
  if (strategy.rebalanceFrequency === 'daily') return true;
  if (strategy.rebalanceFrequency === 'weekly') {
    return !previousExecutionDate || isNewTradingWeek(previousExecutionDate, executionDate);
  }
...
function benchmarkSpyReturn(executionPrices, nextExecutionPrices) {
  const current = executionPrices.get('SPY') ?? executionPrices.get('SP500');
  const next = nextExecutionPrices.get('SPY') ?? nextExecutionPrices.get('SP500');
...
export function buildTickerMarkovStrategyStudy({
  ...
  for (const strategy of strategies) {
    let activeTickers = [];
    let previousExecutionDate = null;
    let cumulative = 1;
    let spyCumulative = 1;
    ...
    for (let index = 1; index < dates.length - 1; index += 1) {
      ...
      if (shouldRebalance(strategy, previousExecutionDate, executionDate)) {
        ...
        previousExecutionDate = executionDate;
      ...
      const spyReturn = benchmarkSpyReturn(executionPrices, nextExecutionPrices);
