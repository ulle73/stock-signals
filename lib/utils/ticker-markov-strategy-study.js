...
function minValue(values) {
  const finiteValues = values.filter((value) => value !== null && value !== undefined && Number.isFinite(value));
  if (!finiteValues.length) return null;
  return Math.min(...finiteValues);
}

function safeRiskRatio(numerator, drawdown) {
  if (numerator === null || numerator === undefined || drawdown === null || drawdown === undefined) return null;
  const denominator = Math.abs(drawdown);
...
export function isNewTradingWeek(previousTradingDate, executionDate) {
...
export function shouldRebalance(strategy, previousTradingDate, executionDate, hasRebalanced) {
...
export function marketAllowsExposure(strategy, tradingSignalByDate, signalDate) {
...
export function pickTickers(strategy, markovRowsForDate) {
...
function summarizeStrategy(strategy, dailyRows, spreadBps) {
  ...
  const maxDrawdown = minValue(dailyRows.map((row) => row.drawdown));
  const spyMaxDrawdown = minValue(dailyRows.map((row) => row.spy_drawdown));
  const equalWeightMaxDrawdown = minValue(dailyRows.map((row) => row.equal_weight_drawdown));
  const excessVsSpy = totalReturn !== null && spyTotalReturn !== null ? totalReturn - spyTotalReturn : null;
  const excessVsEqualWeight = totalReturn !== null && equalWeightTotalReturn !== null
    ? totalReturn - equalWeightTotalReturn
    : null;
  ...
  return {
    ...
    spy_max_drawdown: round(spyMaxDrawdown),
    equal_weight_total_return: round(equalWeightTotalReturn),
    equal_weight_max_drawdown: round(equalWeightMaxDrawdown),
    excess_vs_spy: round(excessVsSpy),
    excess_vs_equal_weight: round(excessVsEqualWeight),
    max_drawdown: round(maxDrawdown),
    return_over_max_drawdown: round(safeRiskRatio(totalReturn, maxDrawdown)),
    excess_vs_spy_over_max_drawdown: round(safeRiskRatio(excessVsSpy, maxDrawdown)),
    excess_vs_equal_weight_over_max_drawdown: round(safeRiskRatio(excessVsEqualWeight, maxDrawdown)),
...
export function getTickerMarkovStrategyByName(strategyName) {
  return DEFAULT_TICKER_MARKOV_STRATEGIES.find((strategy) => strategy.name === strategyName) ?? null;
}
...
export function buildTickerMarkovStrategyStudy({
  ...
  for (const strategy of strategies) {
    ...
    for (let index = 1; index < dates.length - 1; index += 1) {
      ...
      spyPeak = Math.max(spyPeak, spyCumulative);
      equalWeightPeak = Math.max(equalWeightPeak, equalWeightCumulative);
      ...
      const row = {
        ...
        spy_drawdown: spyReturn === null ? null : round(spyCumulative / spyPeak - 1),
        equal_weight_return: round(equalWeightReturn),
        equal_weight_cumulative_return: round(equalWeightCumulative - 1),
        equal_weight_drawdown: round(equalWeightCumulative / equalWeightPeak - 1),
