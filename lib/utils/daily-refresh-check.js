import { buildDailyPriceCoverageDecision } from './daily-price-coverage.js';
...
function buildCoverageTargetStatus(
  priceTickerCountForExpectedDate,
  activeTickerCount,
  minimumPriceCoverageRatio
) {
  const hasObservedCount = priceTickerCountForExpectedDate !== null
    && priceTickerCountForExpectedDate !== undefined
    && priceTickerCountForExpectedDate !== '';
  const hasExpectedCount = activeTickerCount !== null
    && activeTickerCount !== undefined
    && activeTickerCount !== '';
  const observed = Number(priceTickerCountForExpectedDate);
  const expected = Number(activeTickerCount);
  const hasCoverageCounts = hasObservedCount
    && hasExpectedCount
    && Number.isFinite(observed)
    && Number.isFinite(expected)
    && expected > 0;
  const decision = hasCoverageCounts
    ? buildDailyPriceCoverageDecision({
      observedCount: observed,
      expectedCount: expected,
      minimumCoverageRatio: minimumPriceCoverageRatio,
    })
    : null;
  ...
  return {
    ...
    isStale: hasCoverageCounts && !decision.canContinue,
    coveragePercent: decision?.coveragePercent ?? null,
    requiredCoveragePercent: decision?.requiredCoveragePercent ?? null,
    missingCount: decision?.missingCount ?? null,
  };
}
