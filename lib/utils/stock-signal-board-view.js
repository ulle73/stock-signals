...
function regimeBreakoutSignalPill(decision) {
  if (decision === 'trigger') {
    return signalPill('regime_breakout_long', 'Regime breakout long', 'positive');
  }
...
function currentIndicatorPills(row) {
  return [
    ...
    regimeBreakoutSignalPill(row.regime_breakout_decision),
...
function latestIndicatorPills(row) {
  return [
    ...
    regimeBreakoutSignalPill(row.latest_active_regime_breakout_date ? 'trigger' : null),
...
function indicatorPillRank(pill) {
  ...
  if (pill.key.startsWith('regime_breakout_')) return 4;
  if (pill.key.startsWith('breakout_20d_')) return 5;
  if (pill.key.startsWith('tf_sync_')) return 6;
  if (pill.key.startsWith('plce_threshold_')) return 7;
  if (pill.key.startsWith('volume_')) return 8;
  if (pill.key.startsWith('swing_watchlist_')) return 9;
  return 10;
}
...
function latestIndicatorSignal(row) {
  ...
  const regimeBreakoutPill = regimeBreakoutSignalPill(row.latest_active_regime_breakout_date ? 'trigger' : null);
  ...
  if (row.latest_active_regime_breakout_date && regimeBreakoutPill) {
    signals.push({
      date: row.latest_active_regime_breakout_date,
      pill: regimeBreakoutPill,
    });
  }
...
function regimeBreakoutDetailItem(row) {
  const decision = row.regime_breakout_decision ?? 'none';
  const tone = decision === 'trigger'
    ? 'positive'
    : decision === 'blocked'
      ? row.regime_breakout_data_quality_status === 'block' || row.regime_breakout_market_signal === 'risk_off'
        ? 'danger'
        : 'warning'
      : 'neutral';
  ...
  return {
    ...
    signalLabel: decision === 'trigger'
      ? 'Triggered'
      : decision === 'blocked'
        ? 'Blocked'
        : 'Ingen breakout',
    tone,
    isActive: decision === 'trigger',
    statusLabel: decision === 'blocked'
      ? 'Blocked'
      : decision === 'trigger'
        ? 'Aktiv'
        : 'Neutral',
    metrics: [
      metric('Marknad', row.regime_breakout_market_signal ?? null),
      metric('Sektor', row.regime_breakout_sector_signal ?? null),
      metric('RVOL20', toNumber(row.regime_breakout_relative_volume20)),
      metric('RS 63d %ile', toNumber(row.regime_breakout_rs_percentile_63d, 1)),
      metric('Quality', row.regime_breakout_data_quality_status ?? null),
      metric('Score', row.regime_breakout_setup_score ?? null),
      metric('Senast trigger', row.latest_active_regime_breakout_date ?? null),
      metric('Skal', row.regime_breakout_reason_summary ?? row.latest_active_regime_breakout_reason_summary ?? null),
    ].filter((item) => item.value !== null && item.value !== undefined),
  };
}
...
function buildSummary(normalizedRows, rawRows) {
  return {
    ...
    regimeBreakoutCount: normalizedRows.filter((row) => row.currentSignals.some((item) => item.key.startsWith('regime_breakout_'))).length,
    ...
    latestRegimeBreakoutDate: rawRows.reduce((latest, row) => maxDate(latest, row.latest_active_regime_breakout_date), null),
