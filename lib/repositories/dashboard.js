...
export async function getDashboardSnapshot(ticker) {
  const [latestRun, coverage, marketSeries, tickerSnapshot, latestSignal, recentSignals, backtests, positionStatus, macroMatrix, globalManufacturingPmiMatrix] = await Promise.all([
    getLatestFetchRun(),
    getCoverageSummary(),
    ...
    getGlobalManufacturingPmiMatrixSnapshot(),
  ]);
