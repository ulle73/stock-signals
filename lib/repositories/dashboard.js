...
function toFinishedAtMillis(value) {
  if (!value) return 0;
  const millis = new Date(value).getTime();
  return Number.isFinite(millis) ? millis : 0;
}

function toScaledText(value, scale = 1) {
  if (value === null || value === undefined) return null;
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return null;
  return String(Number((numeric * scale).toFixed(6)));
}

export function formatTickerMarkovStrategyName(strategyName) {
  if (!strategyName) {
    return 'Ticker Markov';
  }
  ...
  return `Ticker Markov ${label}`;
}

export function buildDashboardBacktestRows({ backtestRows = [], tickerMarkovRows = [] } = {}) {
  return [...backtestRows, ...tickerMarkovRows].sort((left, right) => {
    const finishedAtDiff = toFinishedAtMillis(right.finished_at) - toFinishedAtMillis(left.finished_at);
    if (finishedAtDiff !== 0) return finishedAtDiff;
    return String(left.code ?? '').localeCompare(String(right.code ?? ''));
  });
}

export function buildTickerMarkovDashboardBacktestRow(row = {}) {
  return {
    code: row.code,
    name: formatTickerMarkovStrategyName(row.strategy_name),
    cagr: row.cagr,
    // Ticker Markov studies store drawdown as a decimal ratio; dashboard rows use percent points.
    max_drawdown: toScaledText(row.max_drawdown, 100),
    time_in_market_pct: row.time_in_market_pct,
...
export async function getTickerMarkovBacktestSummaries(limit = 50) {
  const result = await query(
    `with exposure as (
       select
         strategy_name,
         (
           count(*) filter (where ticker_count > 0)::numeric
           / nullif(count(*)::numeric, 0)
         ) * 100 as time_in_market_pct,
         max(updated_at) as finished_at
       from ticker_markov_strategy_daily
       group by strategy_name
     )
     select
       concat('ticker_markov_', s.strategy_name) as code,
       s.strategy_name,
       case
         when s.total_return is null
           or s.trading_days <= 0
           or (1 + s.total_return) <= 0
           then null
         else ((power(1 + s.total_return, 252.0 / s.trading_days) - 1) * 100)::text
       end as cagr,
       s.max_drawdown::text as max_drawdown,
       e.time_in_market_pct::text as time_in_market_pct,
       s.updated_at as finished_at
     from ticker_markov_strategy_summary s
     left join exposure e
       on e.strategy_name = s.strategy_name
     order by s.updated_at desc, s.strategy_name asc
     limit $1`,
    [limit]
  );

  return result.rows.map(buildTickerMarkovDashboardBacktestRow);
}

export async function getDashboardBacktestSummaries({
  backtestLimit = 50,
  tickerMarkovLimit = 50,
} = {}) {
  const [backtestRows, tickerMarkovRows] = await Promise.all([
    getLatestBacktestSummaries(backtestLimit),
    getTickerMarkovBacktestSummaries(tickerMarkovLimit),
  ]);

  return buildDashboardBacktestRows({ backtestRows, tickerMarkovRows });
}
