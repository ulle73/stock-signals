...
export async function getPlceShortVolumeIndicatorRows() {
  const result = await query(
    `select
       date::text as date,
       plce_short_volume::text as plce_short_volume
     from plce_short_volume_indicator_daily
     order by date asc`
  );
...
export async function getStoredIndicatorRow(ticker, date) {
  const result = await query(
    `select
       ...
       price_zscore_20::text as price_zscore_20,
       price_zscore_avg_20::text as price_zscore_avg_20,
       ibs_value::text as ibs_value,
       rsi14::text as rsi14,
       macd_v::text as macd_v,
       breakout_20d_high::text as breakout_20d_high,
       breakout_20d_low::text as breakout_20d_low,
       plce_threshold_value::text as plce_threshold_value,
       ryd_obv_buy_signal,
       ryd_obv_sell_signal,
       price_zscore_buy_signal,
       price_zscore_sell_signal,
