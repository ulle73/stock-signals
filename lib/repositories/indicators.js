...
export function buildStockDailyIndicatorUpsertStatements(rows, batchSize = INDICATOR_BATCH_SIZE) {
  return chunkArray(rows, batchSize).map((batch) => {
    ...
    const values = batch.map((row, index) => {
      const fieldsPerRow =
        2 +
        INDICATOR_VALUE_FIELDS.length +
        INDICATOR_BOOLEAN_FIELDS.length +
        INDICATOR_TEXT_FIELDS.length;
      ...
      params.push(
        ...
        ...INDICATOR_TEXT_FIELDS.map((field) => row[field] ?? 'none')
      );

      return `($${base + 1}, $${base + 2}, $${base + 3}, $${base + 4}, $${base + 5}, $${base + 6}, $${base + 7}, $${base + 8}, 'adj_close_or_close', $${base + 9}, $${base + 10}, $${base + 11}, $${base + 12}, $${base + 13}, $${base + 14}, $${base + 15}, $${base + 16}, $${base + 17}, $${base + 18}, now())`;
    });

    return {
      sql: `insert into stock_daily_indicators (
        ticker, date, indicator_price, daily_return_pct, avg_volume20, relative_volume20,
        pct_from_52w_high, pct_from_52w_low, price_basis, sma5, sma10, sma20, sma50, sma200,
        ryd_obv, ryd_obv_zscore_80, ryd_obv_buy_signal, ryd_obv_sell_signal, ryd_obv_signal, updated_at
      ) values ${values.join(', ')}
      on conflict (ticker, date) do update set
        ...
        ryd_obv = excluded.ryd_obv,
        ryd_obv_zscore_80 = excluded.ryd_obv_zscore_80,
        ryd_obv_buy_signal = excluded.ryd_obv_buy_signal,
        ryd_obv_sell_signal = excluded.ryd_obv_sell_signal,
        ryd_obv_signal = excluded.ryd_obv_signal,
        updated_at = now()`,
      params,
    };
  });
