...
function scorePmi(value, momChange) {
  if (value === null || value === undefined) return null;
...
function directionScore(value, momChange) {
...
function toObservationRows(rowsByKey, definition, months) {
  ...
  return months.map((periodDate) => {
    ...
    const heatmapScore = scorePmi(value, momChange);
    ...
    return {
      ...
      momChange,
      zScore36m: null,
      directionScore: directionScore(value, momChange),
      heatmapScore,
...
export function buildGlobalManufacturingPmiMatrix(storedRows, { monthCount = DEFAULT_MONTH_COUNT, quarterCount = DEFAULT_QUARTER_COUNT } = {}) {
  ...
  const months = availableMonths.length
    ? availableMonths.slice(-monthCount)
    : Array.from({ length: 1 }, () => latestMonth);
...
export async function getGlobalManufacturingPmiMatrixSnapshot() {
  ...
  try {
    ...
    cachedSnapshot = buildGlobalManufacturingPmiMatrix(storedRows);
    cachedAt = Date.now();
    return cachedSnapshot;
  } catch (error) {
    return null;
