...
function buildStockSignalBoardSql(includeTfSync, includeTickerMarkov, includeRegimeBreakout) {
  return `with latest_indicator_rows as (
    ...
    : ''}
     ${includeRegimeBreakout
    ? `latest_regime_breakout_trigger_rows as (
       select *
       from (
         select
           r.ticker,
           r.date::text as latest_active_regime_breakout_date,
           r.reason_summary as latest_active_regime_breakout_reason_summary,
           row_number() over (
             partition by r.ticker
             order by r.date desc
           ) as row_number
         from regime_gated_breakout_daily r
         where r.decision = 'trigger'
       ) ranked
       where row_number = 1
     ),`
    : ''}
    ...
    board as (
      select
        ...
        least(
          ...
          case when base.regime_breakout_decision = 'trigger' then 4 else 99 end,
          case when base.breakout_20d_signal in ('buy', 'sell') then 5 else 99 end,
          case when base.tf_sync_signal in ('buy', 'sell', 'buy_active', 'sell_active') then 6 else 99 end,
          case when base.plce_threshold_signal = 'buy' then 7 else 99 end,
          case when base.volume_event is not null and base.volume_event <> 'normal' then 8 else 99 end,
          case when base.latest_watchlist_date = base.board_watchlist_date and base.latest_watchlist_bias in ('long', 'short') then 9 else 99 end,
          case when base.ticker_markov_signal in ('bull', 'sell') then 10 else 99 end
        ...
        case
          when
            ...
            or base.regime_breakout_decision = 'trigger'
        ...
        greatest(
          ...
          coalesce(base.latest_active_regime_breakout_date::date, date '1900-01-01'),
      ...
      from (
        select
          ...
          ${includeRegimeBreakout
      ? `regime_breakout.decision as regime_breakout_decision,
          regime_breakout.data_quality_status as regime_breakout_data_quality_status,
          regime_breakout.market_signal as regime_breakout_market_signal,
          regime_breakout.sector_signal as regime_breakout_sector_signal,
          regime_breakout.relative_volume20::text as regime_breakout_relative_volume20,
          regime_breakout.rs_percentile_63d::text as regime_breakout_rs_percentile_63d,
          regime_breakout.setup_score as regime_breakout_setup_score,
          regime_breakout.reason_summary as regime_breakout_reason_summary,
          regime_breakout_history.latest_active_regime_breakout_date,
          regime_breakout_history.latest_active_regime_breakout_reason_summary,`
      : `null as regime_breakout_decision,
          null as regime_breakout_data_quality_status,
          null as regime_breakout_market_signal,
          null as regime_breakout_sector_signal,
          null as regime_breakout_relative_volume20,
          null as regime_breakout_rs_percentile_63d,
          null as regime_breakout_setup_score,
          null as regime_breakout_reason_summary,
          null as latest_active_regime_breakout_date,
          null as latest_active_regime_breakout_reason_summary,`}
      ...
      : ''}
        ${includeRegimeBreakout
      ? `left join regime_gated_breakout_daily regime_breakout
          on regime_breakout.ticker = c.ticker
         and regime_breakout.date = indicator.current_date_value
        left join latest_regime_breakout_trigger_rows regime_breakout_history
          on regime_breakout_history.ticker = c.ticker`
      : ''}
...
async function getBoardFeatureFlags() {
  const [includeTfSync, includeTickerMarkov, includeRegimeBreakout] = await Promise.all([
    hasTable('tf_sync_indicator_daily'),
    hasTable('ticker_markov_daily'),
    hasTable('regime_gated_breakout_daily'),
  ]);

  return { includeTfSync, includeTickerMarkov, includeRegimeBreakout };
}

export async function getStockSignalBoardRows() {
  const { includeTfSync, includeTickerMarkov, includeRegimeBreakout } = await getBoardFeatureFlags();
  const result = await query(buildStockSignalBoardSql(includeTfSync, includeTickerMarkov, includeRegimeBreakout));
  return result.rows;
}
...
export async function getStockSignalBoardSummary() {
  ...
  const result = await query(
    `with board as (
       ${buildStockSignalBoardSql(includeTfSync, includeTickerMarkov, includeRegimeBreakout)}
     )
     select
       ...
       count(*) filter (where regime_breakout_decision = 'trigger')::int as "regimeBreakoutCount",
       ...
       max(board.latest_active_regime_breakout_date) as "latestRegimeBreakoutDate",
