...
async function getOptionalTfSyncRows({ ticker, startDate, latestDate }) {
  try {
    ...
    console.warn('Optional TF Sync chart layer unavailable:', error?.message ?? error);
    return [];
  }
}
...
export async function getChartData({ ticker, period = '1Y', maxRows = DEFAULT_MAX_ROWS }) {
  ...
  const [rowsResult, tfSyncRows, cvolRows, yieldRows] = await Promise.all([
    query(
      `select *
       from (
         select
