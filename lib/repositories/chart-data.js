...
import { getChartBenchmarkDefinition } from '../chart/chart-ticker-options.js';
...
function getOptionalBreadthRows(sector) {
  if (!sector || sector === 'ETF') return Promise.resolve({ sectorRows: [], marketRows: [] });
  return Promise.all([
...
function getOptionalVolatilityRows(ticker, priceSource = 'stock') {
  const priceTable = priceSource === 'benchmark' ? 'benchmark_daily_prices' : 'stock_daily_prices';
...
function getCompanyResult(ticker, benchmarkDefinition) {
  if (benchmarkDefinition) return Promise.resolve({ rows: [benchmarkDefinition] });
  return query(
...
function getLatestPriceResult(ticker, priceSource) {
  const priceTable = priceSource === 'benchmark' ? 'benchmark_daily_prices' : 'stock_daily_prices';
  return query(
...
function getCoreChartRows({ ticker, startDate, maxRows, priceSource }) {
  if (priceSource === 'benchmark') {
    return query(
      `select *
       from (
         select
           ...
           null::text as sma50,
           null::text as sma200,
           null::text as ryd_obv,
           null::text as ryd_obv_zscore_80,
           null::boolean as ryd_obv_buy_signal,
           null::boolean as ryd_obv_sell_signal,
           null::text as ryd_obv_signal,
           null::text as plce_threshold_value,
           null::boolean as plce_threshold_buy_signal,
           null::text as plce_threshold_signal
...
export async function getChartData({ ticker, period = '1Y', maxRows = DEFAULT_MAX_ROWS }) {
  ...
  const benchmarkDefinition = getChartBenchmarkDefinition(safeTicker);
  const priceSource = benchmarkDefinition?.price_source ?? 'stock';

  const [companyResult, latestResult] = await Promise.all([
    getCompanyResult(safeTicker, benchmarkDefinition),
    getLatestPriceResult(safeTicker, priceSource),
  ]);
  ...
  const [rowsResult, tfSyncRows, cvolRows, yieldRows, gexDexRows, relativeStrengthRows, breadthRows, volatilityRows, earningsRows] = await Promise.all([
    getCoreChartRows({ ticker: safeTicker, startDate, maxRows: safeMaxRows, priceSource }),
    ...
    getOptionalVolatilityRows(safeTicker, priceSource),
