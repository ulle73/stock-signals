...
function toColorBucket(score) {
  ...
  if (score >= 0.75) return 'strong_positive';
...
function getDirection(definitionKey) {
  return EUROPE_GROWTH_INDICATOR_DEFINITIONS.find((item) => item.key === definitionKey)?.direction ?? 'higher_is_better';
}

function scoreIndicator(value, momChange, direction) {
  if (value === null || value === undefined) return null;

  if (direction === 'pmi') {
    if (value >= 52 && (momChange === null || momChange >= 0)) return 1;
    if (value >= 50 && (momChange === null || momChange >= 0)) return 0.5;
    if (value >= 50) return 0.15;
    if (value < 48 && (momChange === null || momChange <= 0)) return -1;
    if (value < 50 && momChange !== null && momChange > 0) return -0.15;
    return -0.5;
  }
...
function directionScore(value, momChange, direction) {
  if (value === null || value === undefined) return 0;
  if (direction === 'pmi') {
    if (value >= 50 && (momChange === null || momChange >= 0)) return 1;
    if (value < 50 && momChange !== null && momChange < 0) return -1;
    return 0;
  }
  if (momChange === null || momChange === 0) return 0;
...
function toObservationRows(rowsByKey, definition, months) {
  ...
  return months.map((periodDate) => {
    ...
    const momChange = value !== null && previousValue !== null ? normalizeNumber(value - previousValue) : null;
    const heatmapScore = scoreIndicator(value, momChange, definition.direction);
    ...
    return {
      ...
      momChange,
      zScore36m: null,
      directionScore: directionScore(value, momChange, definition.direction),
      heatmapScore,
...
export function buildEuropeGrowthMatrix(storedRows, { monthCount = DEFAULT_MONTH_COUNT, quarterCount = DEFAULT_QUARTER_COUNT } = {}) {
  ...
  const months = availableMonths.length ? availableMonths.slice(-monthCount) : [latestMonth];
  ...
  const rows = EUROPE_GROWTH_INDICATOR_DEFINITIONS.map((definition) => {
    ...
    const quarterlyCells = quarters.map((quarter) => {
      ...
      return {
        ...
        colorBucket: toColorBucket(heatmapScore ?? direction),
      };
    });
    ...
    return {
      ...
      label: definition.label,
      category: 'europe_growth',
      direction: getDirection(definition.key),
      cells,
      quarterlyCells,
  ...
  const summaryByMonth = months.map((periodDate, index) => {
    const validCells = rows.map((row) => row.cells[index]).filter((cell) => cell.transformedValue !== null);
...
export async function getEuropeGrowthMatrixSnapshot() {
  ...
  try {
    ...
    cachedSnapshot = buildEuropeGrowthMatrix(storedRows);
    cachedAt = Date.now();
    return cachedSnapshot;
  } catch (error) {
    return null;
