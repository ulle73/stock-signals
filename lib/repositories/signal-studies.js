...
async function listExistingColumns(tableNames) {
  if (!tableNames.length) {
    return new Map();
  }
...
function buildFieldAvailability(field, existingTables, columnsByTable) {
  const tableExists = existingTables.has(field.sourceTable);
  const columns = columnsByTable.get(field.sourceTable) ?? new Set();
  const requiredColumns = [field.dateField];

  if (!field.selectExpression) {
    requiredColumns.push(field.valueField);
  }

  if (field.scope === 'ticker' && field.tickerField) {
    requiredColumns.push(field.tickerField);
  }

  const missingColumns = tableExists
    ? requiredColumns.filter((column) => !columns.has(column))
    : requiredColumns;

  return {
    ...field,
    isAvailable: tableExists && missingColumns.length === 0,
    tableExists,
    missingColumns,
  };
}
...
export async function listSignalStudyFieldsWithAvailability(fields) {
  const tableNames = [...new Set(fields.map((field) => field.sourceTable))];
  const [existingTables, columnsByTable] = await Promise.all([
    listExistingTables(tableNames),
    listExistingColumns(tableNames),
  ]);

  return fields.map((field) => buildFieldAvailability(field, existingTables, columnsByTable));
}
