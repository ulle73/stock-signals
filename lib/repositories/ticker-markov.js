...
export async function getLatestTickerMarkovRows({ limit = 50, signal = null } = {}) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 50, 500));
  const params = [safeLimit];
  const signalFilter = signal ? 'and signal = $2' : '';
  if (signal) {
    params.push(signal);
  }
  ...
  const result = await query(
    `with latest_date as (
     ...
     where date = (select date from latest_date)
       ${signalFilter}
     order by
       case when $${signal ? 2 : 1} = 'sell' then rank_sell else rank_bull end asc nulls last,
       ticker asc
     limit $1`,
    params
  );
