...
const BREADTH_BATCH_SIZE = 100;

export function buildMarketBreadthUpsertStatements(rows, batchSize = BREADTH_BATCH_SIZE) {
  return chunkArray(rows, batchSize).map((batch) => {
    ...
    return {
      sql: `insert into market_breadth_daily (
        date,
        active_ticker_count,
        advancers,
        decliners,
...
export async function upsertMarketBreadthDaily(rows) {
  if (!rows.length) return 0;

  const statements = buildMarketBreadthUpsertStatements(rows);
