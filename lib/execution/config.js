...
function parseRebalancePolicy(rawValue, defaultValue = 'buffered_band_rebalance') {
  const value = String(rawValue ?? defaultValue).trim();
  const allowed = new Set([
    'buffered_band_rebalance',
    'full_rebalance_exact',
  ]);

  if (!allowed.has(value)) {
    throw new Error(`Invalid EXECUTION_REBALANCE_POLICY: ${rawValue}`);
  }
...
function buildExecutionPolicyConfig(env, prefix = 'EXECUTION') {
  return {
    rebalancePolicy: parseRebalancePolicy(env[`${prefix}_REBALANCE_POLICY`] ?? env.EXECUTION_REBALANCE_POLICY),
    targetGrossExposurePct: parsePositiveNumber(
      `${prefix}_TARGET_GROSS_EXPOSURE_PCT`,
      env[`${prefix}_TARGET_GROSS_EXPOSURE_PCT`] ?? env.EXECUTION_TARGET_GROSS_EXPOSURE_PCT,
      95
    ),
    cashBufferPct: parsePositiveNumber(
      `${prefix}_CASH_BUFFER_PCT`,
      env[`${prefix}_CASH_BUFFER_PCT`] ?? env.EXECUTION_CASH_BUFFER_PCT,
      5
    ),
    rebalanceBandRelative: parsePositiveNumber(
      `${prefix}_REBALANCE_BAND_RELATIVE`,
      env[`${prefix}_REBALANCE_BAND_RELATIVE`] ?? env.EXECUTION_REBALANCE_BAND_RELATIVE,
      0.25
    ),
    rebalanceBandAbsolutePct: parsePositiveNumber(
      `${prefix}_REBALANCE_BAND_ABSOLUTE_PCT`,
      env[`${prefix}_REBALANCE_BAND_ABSOLUTE_PCT`] ?? env.EXECUTION_REBALANCE_BAND_ABSOLUTE_PCT,
      1
    ),
    minOrderNotionalUsd: parsePositiveNumber(
      `${prefix}_MIN_ORDER_NOTIONAL_USD`,
      env[`${prefix}_MIN_ORDER_NOTIONAL_USD`] ?? env.EXECUTION_MIN_ORDER_NOTIONAL_USD,
      100
    ),
    minOrderEquityPct: parsePositiveNumber(
      `${prefix}_MIN_ORDER_EQUITY_PCT`,
      env[`${prefix}_MIN_ORDER_EQUITY_PCT`] ?? env.EXECUTION_MIN_ORDER_EQUITY_PCT,
      0.5
    ),
    maxRebalanceTurnoverPct: parsePositiveNumber(
      `${prefix}_MAX_REBALANCE_TURNOVER_PCT`,
      env[`${prefix}_MAX_REBALANCE_TURNOVER_PCT`] ?? env.EXECUTION_MAX_REBALANCE_TURNOVER_PCT,
      30
    ),
  };
}
...
export function getExecutionConfig(env = process.env) {
  return {
    ...
    ...buildExecutionPolicyConfig(env),
  };
}
...
export function getMarkovPaperAccountConfigs(env = process.env) {
  ...
  return accountIds.map((accountId) => {
    ...
    return {
      ...
      ...buildExecutionPolicyConfig(env, prefix),
    };
  });
}
