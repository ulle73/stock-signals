...
function roundToWholeShareQuantity(value) {
...
function buildProposal(intent, brokerState) {
  ...
  if (intent.target_state === 'long' || intent.target_state === 'short') {
    const targetQtyMagnitude = referencePrice
      ? Math.abs(targetPositionNotional) / referencePrice
      : null;
    const targetQtySigned = referencePrice
      ? ((intent.target_state === 'short' ? -1 : 1) * (
        intent.target_state === 'short'
          ? roundToWholeShareQuantity(targetQtyMagnitude)
          : roundToQuantity(targetQtyMagnitude)
      ))
      : null;
