...
export default function DashboardTopNav({ activeItem = 'Översikt', updatedLabel }) {
  return (
    <header className="market-topbar">
      ...
      <nav className="market-top-nav" aria-label="Huvudnavigation">
        {navItems.map((item) => (item.href ? (
          <a className={item.label === activeItem ? 'is-active' : undefined} href={item.href} key={item.label}>
