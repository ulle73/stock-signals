...
function formatDate(value) {
  if (!value) return '—';
  return new Intl.DateTimeFormat('sv-SE', { dateStyle: 'medium', timeZone: 'UTC' })
...
export default function CrosshairLegend({
  ...
  return (
    <div className="chart-crosshair-legend" aria-live="polite">
      <div className="chart-crosshair-date"><span>Datum</span><strong>{formatDate(point?.time)}</strong></div>
      <dl className="chart-crosshair-values">
        ...
        {overlays.map((key) => (
          <div key={key}>
            <dt><i aria-hidden="true" style={{ background: CHART_SERIES[key].color }} />{CHART_SERIES[key].label}</dt>
