import { getGrowthDataBaseEffectsMatrixSnapshot } from '../lib/repositories/macro-matrix-growth-data-base-effects.js';
...
export default async function GrowthDataBaseEffectsSection() {
  const matrix = await getGrowthDataBaseEffectsMatrixSnapshot();
  ...
  return (
    <section className="card macro-matrix-card">
      <div className="macro-matrix-topline">
        <div>
          <p className="section-kicker">Macro · Growth Data Base Effects</p>
          <h2>Tillväxtdata faller tillbaka</h2>
          <p className="hero-copy compact">
            Growth momentum-matrix som separerar faktisk försämring från YoY-base effects med easy/normal/hard comparison-regim.
          </p>
        </div>
        <div className="macro-matrix-status">
          <span className={`mini-pill ${tone(latest?.growthBaseEffectRiskAction)}`}>{status(latest?.growthBaseEffectRiskAction)}</span>
          <strong>{n(latest?.growthMomentumScore, 2)}</strong>
          <span>score · {month(latest?.periodDate)}</span>
        </div>
      </div>

      <div className="macro-matrix-metric-strip">
        <div className="macro-mini-stat"><span>Regim</span><strong>{status(latest?.growthBaseEffectRegime)}</strong></div>
        <div className="macro-mini-stat"><span>Base effect</span><strong>{status(latest?.baseEffectRegime)}</strong></div>
        <div className="macro-mini-stat"><span>Confidence</span><strong>{status(latest?.confidence)}</strong></div>
        <div className="macro-mini-stat"><span>Rader live</span><strong>{matrix.availableRowCount ?? 0}/{matrix.totalRowCount ?? 0}</strong></div>
      </div>
      ...
      <div className="macro-matrix-scroll">
        <table className="macro-matrix-table">
          <thead>
            <tr>
              <th>Growth Data / Base Effects</th>
      ...
      <p className="footnote">
        Base-effect score beräknas på YoY-radernas 12-månaders jämförelsebas. Hard comparisons sänker confidence för negativa tillväxtsignaler.
      </p>
    </section>
  );
}
