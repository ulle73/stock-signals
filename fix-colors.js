...
function fixColors(filePath) {
  ...
  // Hardcoded chart borders or backgrounds in globals.css
  ...
  // Special background overrides for the cards
  content = content.replace(/background:\s*linear-gradient\(180deg,\s*rgba.*?var\(--card-strong\);/gs, 'background: var(--hero-bg);');
...
// If macro-matrix-colors.css exists and needs fixes, we can do it later.
