...
test('normalizeChartRows sorts ascending and keeps the last duplicate date', () => {
  const payload = normalizeChartRows({
    ...
    period: '1Y',
    rows: [
      { date: '2026-07-10', open: '210', high: '214', low: '209', close: '213', adj_close: '213', volume: '10', sma20: '205' },
      { date: '2026-07-09', open: '208', high: '211', low: '207', close: '210', adj_close: '210', volume: '9', sma20: null },
      { date: '2026-07-10', open: '210', high: '215', low: '209', close: '214', adj_close: '214', volume: '11', sma20: '206' },
    ],
  });
  ...
  assert.equal(payload.dailyChange, 4);
...
test('normalizeChartRows adjusts all OHLC values to the adjusted-close scale', () => {
  const payload = normalizeChartRows({
    ...
    period: 'ALL',
    rows: [{
      date: '2020-08-31', open: '500', high: '520', low: '480', close: '500',
      adj_close: '125', volume: '100', sma20: '120',
    }],
  });
...
test('normalizeChartRows preserves stored RYD OBV values and warmup gaps', () => {
  const payload = normalizeChartRows({
    ...
    rows: [
      {
        date: '2026-07-09', open: '208', high: '211', low: '207', close: '210', adj_close: '210', volume: '9',
        ryd_obv: '1000000', ryd_obv_zscore_80: null, ryd_obv_buy_signal: false,
        ryd_obv_sell_signal: false, ryd_obv_signal: 'none',
      },
      {
        date: '2026-07-10', open: '210', high: '215', low: '209', close: '214', adj_close: '214', volume: '11',
        ryd_obv: '1250000.5', ryd_obv_zscore_80: '-2.6', ryd_obv_buy_signal: true,
        ryd_obv_sell_signal: false, ryd_obv_signal: 'buy',
      },
    ],
  });
  ...
  assert.deepEqual(payload.bars[0], {
    time: '2026-07-09', open: 208, high: 211, low: 207, close: 210, volume: 9,
    ryd_obv: 1000000, ryd_obv_buy_signal: false, ryd_obv_sell_signal: false, ryd_obv_signal: 'none',
  });
  assert.deepEqual(payload.bars[1], {
    time: '2026-07-10', open: 210, high: 215, low: 209, close: 214, volume: 11,
    ryd_obv: 1250000.5, ryd_obv_zscore_80: -2.6, ryd_obv_buy_signal: true,
    ryd_obv_sell_signal: false, ryd_obv_signal: 'buy',
  });
});
...
test('normalizeChartRows preserves stored TF Sync states and sanitizes unknown labels', () => {
  const payload = normalizeChartRows({
    ...
    rows: [
      {
        date: '2026-07-10', open: '210', high: '215', low: '209', close: '214', adj_close: '214', volume: '11',
        tf_sync_buy_signal: 'true', tf_sync_sell_signal: false, tf_sync_buy_active: 1,
        tf_sync_sell_active: 0, tf_sync_signal: 'buy',
      },
      {
        date: '2026-07-11', open: '214', high: '216', low: '212', close: '215', adj_close: '215', volume: '12',
        tf_sync_buy_signal: false, tf_sync_sell_signal: true, tf_sync_buy_active: false,
        tf_sync_sell_active: true, tf_sync_signal: 'unexpected',
      },
    ],
  });

  assert.equal(payload.bars[0].tf_sync_buy_signal, true);
  assert.equal(payload.bars[0].tf_sync_buy_active, true);
  assert.equal(payload.bars[0].tf_sync_signal, 'buy');
  assert.equal(payload.bars[1].tf_sync_sell_signal, true);
  assert.equal(payload.bars[1].tf_sync_sell_active, true);
